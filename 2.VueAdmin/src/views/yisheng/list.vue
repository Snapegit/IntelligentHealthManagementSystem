...
<template>
	<div>
		<div class="app-contain">
				...
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.yishenggonghao" placeholder="医生工号"
								clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.zhengzhimianmao" placeholder="政治面貌"
								clearable>
							</el-input>
						</div>
					</div>
			...
			<el-table
				...
				<el-table-column
					 ...
					 align="left" 
					 header-align="left"
					 prop="wenzhenzhuangtai"
					label="问诊状态">
					<template #default="scope">
						{{scope.row.wenzhenzhuangtai}}
					</template>
				</el-table-column>
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('yisheng','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('yisheng','问诊')" type="success" @click="jiankangwenzhenCrossAddOrUpdateHandler(scope.row,'cross','','wenzhenzhuangtai','不可问诊','不可问诊,可问诊'.split(',')[0])">问诊</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<jiankangwenzhenFormModel ref="jiankangwenzhenFormModelRef" @formModelChange="formModelChange"></jiankangwenzhenFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.zhengzhimianmao&&searchQuery.value.zhengzhimianmao!=''){
			params['zhengzhimianmao'] = '%' + searchQuery.value.zhengzhimianmao + '%'
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
	...
	import jiankangwenzhenFormModel from '@/views/jiankangwenzhen/formModel'
	const jiankangwenzhenFormModelRef = ref(null)
    const jiankangwenzhenCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			jiankangwenzhenFormModelRef.value.init(row.id,'cross','问诊',row,'yisheng',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
