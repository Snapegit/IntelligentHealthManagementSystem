	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.wenjuanbiaoti" placeholder="问卷标题"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('wenjuandiaocha','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('wenjuandiaocha','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('wenjuandiaocha','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('wenjuandiaocha','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				ref="table"
				v-if="btnAuth('wenjuandiaocha','查看')"
				:data="list"
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('wenjuandiaocha','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('wenjuandiaocha','问卷回答')" type="success" @click="wenjuanhuidaCrossAddOrUpdateHandler(scope.row,'cross','','','')">问卷回答</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<wenjuanhuidaFormModel ref="wenjuanhuidaFormModelRef" @formModelChange="formModelChange"></wenjuanhuidaFormModel>
	</div>
</template>
<script setup>
	...
	//基础信息
	const tableName = 'wenjuandiaocha'
	const formName = '问卷调查'
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.wenjuanbiaoti&&searchQuery.value.wenjuanbiaoti!=''){
			params['wenjuanbiaoti'] = '%' + searchQuery.value.wenjuanbiaoti + '%'
		}
	...
	import wenjuanhuidaFormModel from '@/views/wenjuanhuida/formModel'
	const wenjuanhuidaFormModelRef = ref(null)
    const wenjuanhuidaCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			wenjuanhuidaFormModelRef.value.init(row.id,'cross','问卷回答',row,'wenjuandiaocha',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
